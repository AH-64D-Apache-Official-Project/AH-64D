...
params ["_heli", "_rotorIndex", "_pivot", "_rot", "_type", "_dir", "_numBlades", "_numElements", "_mastLength", "_gearRatio", "_flapTimeConst", "_delta3", "_airfoilTable", "_bladeCutout", "_bladeLength", "_bladeChord", "_bladeTwist", "_bladeMass", "_pitchMin", "_pitchMid", "_pitchMax", "_rollMin", "_rollMid", "_rollMax", "_collMin", "_collMid", "_collMax", "_animSource"];
...
private _rotorAzimuth = 0.0;//(_heli animationSourcePhase _animSource) * 360.0 * (if (_dir == CW) then { 1 } else { -1 });
...
for "_bladeIndex" from 0 to 3 do {
	private _psi = _rotorAzimuth + (90.0 * _bladeIndex);
