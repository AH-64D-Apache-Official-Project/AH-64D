...
private _rotation       = _heli getVariable "fza_sfmplus_fuselageTopRotation";
//private _dragCoefTable  = _heli getVariable "fza_sfmplus_fuselageTopDragCoefTable";
private _airfoilTable   = getArray (_sfmPlusConfig >> "airfoilTable01");
private _count          = _heli getVariable "fza_sfmplus_fuselageTopCount";
private _coords         = _heli getVariable "fza_sfmplus_fuselageTop";
...
for "_i" from 0 to (_count - 1) do {
    ...
    // Lift uses total relative wind speed, capped at 120 m/s to prevent V^2 divergence above VNE.
    private _v            = (vectorMagnitude _relWind) min 120.0;
    private _lift         = _CL * 0.5 * _rho * _area * (_v * _v);

    //Drag coefficient
    // Drag uses only the Z (vertical) component — the top fuselage face only generates drag from
    // velocity perpendicular to it. Capped independently so unrelated forward speed has no effect.
    private _CD          = [_airfoilTable, _aoa] call fza_fnc_linearInterp select 2;
    private _rZ           = [_relWind select 2, -120.0, 120.0] call BIS_fnc_clamp;
    private _drag         = _CD * 0.5 * _rho * _area * (_rZ * _rZ);
