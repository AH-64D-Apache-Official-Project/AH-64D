...
for "_i" from 0 to (_count - 1) do {
    ...
    //Drag coefficient
    // Drag uses only the X (lateral) component — the side fuselage face only generates drag from
    ...
    private _rX           = [_relWind select 0, -120.0, 120.0] call BIS_fnc_clamp;
    private _drag         = _CD * 0.5 * _rho * _area * (_rX * _rX);
