class CfgSounds
{
    ///STARTING CLICKABLE SEQUENCE SOUNDS////
    
   /* class fza_ah64_apustart_3D
    {
        name = "fza_ah64_apustart_3D";
        sound[] = {"fza_ah64_controls\data\audio\APU_Start.ogg",1,1,300};
        titles[]={};
    };
    class fza_ah64_apu_loop_3D
    {
        name = "fza_ah64_apu_loop_3D";
        sound[] = {"fza_ah64_controls\data\audio\APU_Loop.ogg",1,1,300};
        titles[]={};
    };
    class fza_ah64_apustop_3D
    {
        name = "fza_ah64_apustop_3D";
        sound[] = {"fza_ah64_controls\data\audio\APU_End.ogg",1,1,300};
        titles[]={};
    };
    class fza_ah64_bat_loop_3D
    {
        name = "fza_ah64_bat_loop_3D";
        sound[] = {"fza_ah64_controls\data\audio\Avionics.ogg",10,1,10};
        titles[]={};
    };
    class fza_ah64_estart_3D
    {
        name = "fza_ah64_estart_3D";
        sound[] = {"fza_ah64_controls\data\audio\Start_Ext.ogg",1,1,300};
        titles[]={};
    };*/
    class fza_ah64_fake_3D
    {
        name = "fza_ah64_fake_3D";
        sound[] = {"A3\Sounds_F\dummysound",1,1,10};
        titles[]={};
    };

    ///GENERIC FLIP SOUNDS///
    class fza_ah64_button_click1
    {
        name = "fza_ah64_button_click1";
        sound[] = {"fza_ah64_controls\data\audio\Button1.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_button_click2
    {
        name = "fza_ah64_button_click2";
        sound[] = {"\fza_ah64_controls\data\audio\Button2.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_knob
    {
        name = "fza_ah64_knob";
        sound[] = {"\fza_ah64_controls\data\audio\Knob.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_monoclebox
    {
        name = "fza_ah64_monoclebox";
        sound[] = {"\fza_ah64_controls\data\audio\HDU.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_battery
    {
        name = "fza_ah64_battery";
        sound[] = {"\fza_ah64_controls\data\audio\Battery.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_apubutton
    {
        name = "fza_ah64_apubutton";
        sound[] = {"\fza_ah64_controls\data\audio\APU_Button.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_switch_flip1
    {
        name = "fza_ah64_switch_flip1";
        sound[] = {"\fza_ah64_controls\data\audio\Switch_Flip1.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_switch_flip2
    {
        name = "fza_ah64_switch_flip2";
        sound[] = {"\fza_ah64_controls\data\audio\Switch_Flip2.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_switch_flip3
    {
        name = "fza_ah64_switch_flip3";
        sound[] = {"\fza_ah64_controls\data\audio\Switch_Flip3.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_switch_flip4
    {
        name = "fza_ah64_switch_flip4";
        sound[] = {"\fza_ah64_controls\data\audio\Switch_Flip4.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_button_rotary
    {
        name = "fza_ah64_button_rotary";
        sound[] = {"\fza_ah64_controls\data\audio\Rotary.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_throttle_idle
    {
        name = "fza_ah64_throttle_idle";
        sound[] = {"\fza_ah64_controls\data\audio\throttle_lever_idle.ogg",1,1,1};
        titles[]={};
    };
    
    ///END OF GENERIC FLIP SOUNDS///

    ///DOOR SOUNDS///
    class fza_ah64_rh_dooropen1
    {
        name = "fza_ah64_rh_dooropen1";
        sound[] = {"\fza_ah64_controls\data\audio\Door_Open.ogg",1,1,1};
        titles[]={};
    };
    class fza_ah64_rh_doorclose1
    {
        name = "fza_ah64_rh_doorclose1";
        sound[] = {"\fza_ah64_controls\data\audio\Door_Close.ogg",1,1,1};
        titles[]={};
    };
    ///END OF DOOR SOUNDS///
    };