...
if(local _hostile && !(local _ac)) then
{
	...
	while {(alive _missile) && (alive _ac) && (_mistheta > 300 || _mistheta < 60)} do
	{
		_reldir = ((getposasl _ac select 0) - (getposasl _missile select 0)) atan2 ((getposasl _ac select 1) - (getposasl _missile select 1));
		if (_reldir < 0) then
		{
...
{
	waituntil {!(alive _missile) || !(alive _ac) || (fza_ah64_irjon > 0) || fza_ah64_curflrln > 0};
...
waitUntil {_missile distance _ac < _scramdist};
