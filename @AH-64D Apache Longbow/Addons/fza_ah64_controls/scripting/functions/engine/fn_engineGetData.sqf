...
switch (_state) do {
	...
	case "OFFIDLE": {
		private _result = [_offIdleTransition, time - _stateStartTime] call fza_fnc_linearInterp;
	...
	case "IDLEOFF": {
		private _result = [_idleOffTransition, time - _stateStartTime] call fza_fnc_linearInterp;
		_npMul = _result select 1;
		_ng = _result select 2;
		_tgt = _result select 3;
		_oil = _result select 4;
	...
	case "IDLEFLY": {
		private _result = [_idleFlyTransition, time - _stateStartTime] call fza_fnc_linearInterp;
	...
	case "FLYIDLE": {
		private _result = [_flyIdleTransition, time - _stateStartTime] call fza_fnc_linearInterp;
		_ng = _result select 1;
		_tgt = _result select 2;
		_oil = _result select 3;
		_torqueMul = 100;
