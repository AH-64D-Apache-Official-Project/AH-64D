...
do {
    ...
    waituntil {
        !(player == driver _heli);
        if (inputaction "nextCM" > 0.5 && _cmpressed == 0) then {
            fza_ah64_cmsel = fza_ah64_cmsel + 1;
            _cmpressed = 1;
        };
        if (inputaction "nextCM" < 0.5 && _cmpressed == 1) then {
            _cmpressed = 0;
        };
        ...
        if (fza_ah64_tiron) then {
            fza_ah64_pnvselev = (-1.5 * (inputaction "LookDownCont")) + (0.5 * (inputaction "LookUpCont")) + 0.5;
            fza_ah64_pnvsdir = (-1 * (inputaction "LookLeftCont")) + (inputaction "LookRightCont");
        };

        if (fza_ah64_pnvsdir > 0.75) then {
            fza_ah64_pnvsdir = 0.75;
        };
        if (fza_ah64_pnvsdir < -0.75) then {
            fza_ah64_pnvsdir = -0.75;
        };

        if (fza_ah64_estate > -1 && !(isengineon _heli)) then {
            fza_ah64_estate = fza_ah64_estate - 0.02;
            fza_ah64_pnvsdir = fza_ah64_estate;
        };
        if ((fza_ah64_apuon == 1 || isengineon _heli) && fza_ah64_estate < 0) then {
            fza_ah64_estate = fza_ah64_estate + 0.02;
            fza_ah64_pnvsdir = fza_ah64_estate;
        };
        if (fza_ah64_estate < -0.98 && !(isengineon _heli)) then {
            fza_ah64_pnvsdir = -1;
            fza_ah64_estate = -1;
        };
        if (fza_ah64_estate > -0.02 && (isengineon _heli)) then {
            fza_ah64_estate = 0;
        };
