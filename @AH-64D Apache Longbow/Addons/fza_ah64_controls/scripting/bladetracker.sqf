...
if(_collective > 0.1) then {_collective = 0.1;};
if(_collective < -0.6) then {_collective = -0.6;};
_rudder = (-0.5*(inputAction "HeliRudderLeft")) + (0.5*(inputAction "HeliRudderRight"));
_cyclicdir = _leftright atan2 _forback;
if(_cyclicdir < 0) then {_cyclicdir = _cyclicdir + 360;};
...
_b2phase = (_heli animationphase "mainRotor")+(0.25+_cyclicdir);
if(_b2phase > 1) then {_b2phase = _b2phase - 1;};
_b2phase = (cos(360*_b2phase))*_magnitude;
_b3phase = (_heli animationphase "mainRotor")+(0.5+_cyclicdir);
if(_b3phase > 1) then {_b3phase = _b3phase - 1;};
_b3phase = (cos(360*_b3phase))*_magnitude;
_b4phase = (_heli animationphase "mainRotor")+(0.75+_cyclicdir);
if(_b4phase > 1) then {_b4phase = _b4phase - 1;};
_b4phase = (cos(360*_b4phase))*_magnitude;
//scissorarm1
_s1phase = (_heli animationphase "mainRotor")+(0.875+_cyclicdir);
if(_s1phase > 1) then {_s1phase = _s1phase - 1;};
_s1phase = (cos(360*_s1phase))*_magnitude;
//scissorarm2
_s2phase = (_heli animationphase "mainRotor")+(0.375+_cyclicdir);
if(_s2phase > 1) then {_s2phase = _s2phase - 1;};
