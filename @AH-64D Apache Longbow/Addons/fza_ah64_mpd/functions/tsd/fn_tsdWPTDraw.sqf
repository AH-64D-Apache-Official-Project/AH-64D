...
[_heli, MFD_INDEX_OFFSET(MFD_IND_TSD_WPT_VARIANT), _state get "subPageVarPage" select 1] call fza_mpd_fnc_updateMfdValue;
[_heli, MFD_INDEX_OFFSET(MFD_TEXT_IND_TSD_WPT_CURRENT_POINT), _currentPointStr] call fza_mpd_fnc_updateMfdText;

private _pointDetails = [_heli, _currentPoint, POINT_GET_FULL] call fza_dms_fnc_pointGetValue;

if (isNil "_pointDetails") then {
    [_heli, MFD_INDEX_OFFSET(MFD_TEXT_IND_TSD_WPT_DETAILS_1), format["?01 %1 AAA ETE 00:00:00 ETA 00:00:00L", POINT_DEFAULTIDENT]] call fza_mpd_fnc_updateMfdText;
    [_heli, MFD_INDEX_OFFSET(MFD_TEXT_IND_TSD_WPT_DETAILS_2), "W84 47 13S FU  XXXX XXXX XXX* XX.X KM"] call fza_mpd_fnc_updateMfdText;
    [_heli, MFD_INDEX_OFFSET(MFD_TEXT_IND_TSD_WPT_DETAILS_3), "NXX XX.XX WXXX XX.XX  XXXX FT XX.X NM"] call fza_mpd_fnc_updateMfdText;
} else {
    //Line 1
    ...
    [_heli, MFD_INDEX_OFFSET(MFD_TEXT_IND_TSD_WPT_DETAILS_1), _wptDetails_1] call fza_mpd_fnc_updateMfdText;
    [_heli, MFD_INDEX_OFFSET(MFD_TEXT_IND_TSD_WPT_DETAILS_2), _wptDetails_2] call fza_mpd_fnc_updateMfdText;
    [_heli, MFD_INDEX_OFFSET(MFD_TEXT_IND_TSD_WPT_DETAILS_3), _wptDetails_3] call fza_mpd_fnc_updateMfdText;
};

private _variant = _state get "subPageVarPage" select 1;
switch (_variant) do {
    case 1: {
        [_heli, MFD_INDEX_OFFSET(MFD_IND_TSD_WPT_ADD_TYPE), _state get "addType"] call fza_mpd_fnc_updateMfdValue;
    };
};
