class VABs_Mode_Msl {
    ...
    class hide_mode {
        condition = C_COND(C_AND(C_EQ(C_MPD_USER(MFD_IND_WPN_MSL_Traj_Menu), 0), C_NOT(C_MPD_USER(MFD_IND_WPN_SELECTED_MSL_TYPE))));
...
class VABs_Mode_Msl_SAL {
    ...
    class Hide_type {
        condition = C_COND(C_AND(C_EQ(C_MPD_USER(MFD_IND_WPN_SELECTED_WPN), 3), C_EQ(C_MPD_USER(MFD_IND_WPN_MSL_Traj_Menu), 0)));
        ...
        class TrajBox_3 {
            condition = C_COND(C_EQ(C_MPD_USER(MFD_IND_WPN_MSL_Traj), 1));
            MPD_BOX_L(TrajBox_3, MPD_POS_BUTTON_R_X, MPD_POS_BUTTON_LR_3_Y + 0.5*MPD_TEXT_HEIGHT, 3)
        };
        class TrajBox_2 {
            condition = C_COND(C_NOT(C_EQ(C_MPD_USER(MFD_IND_WPN_MSL_Traj), 1)));
    ...
    class TrajectorySelect {
        condition = C_COND(C_EQ(C_MPD_USER(MFD_IND_WPN_MSL_Traj_Menu), 1));
        class Box {
            color[] = {1,1,1,1};
            type = line;
            width = 3;
        ...
        class Selected_traj_HI {
            condition = C_COND(C_EQ(C_MPD_USER(MFD_IND_WPN_MSL_Traj), 3));
            MPD_BOX_L(Traj_select_3, MPD_POS_BUTTON_R_X, MPD_POS_BUTTON_LR_1_Y, 2)
        };
        class Selected_traj_LO {
            condition = C_COND(C_EQ(C_MPD_USER(MFD_IND_WPN_MSL_Traj), 2));
