    ...
    switch (_type) do {
        case ("missile" in _type): {
            ...
            if ([_seekerhead, 8] call BIS_fnc_bitwiseAND != 0) then {
                ...
                [_heli, _launcher] call fza_ase_fnc_classification params ["_soundclass","_iconClass"];
                _rwrObjects pushBack [ASE_LNC, getpos _object, _iconClass];
                continue;
            };
        };
        case ("locked" in _type): {
            [_heli, _object] call fza_ase_fnc_classification params ["_soundclass","_iconClass"];
            _rwrObjects pushBack [ASE_TRK, getpos _object, _iconClass];
            _rwrAudio pushback [_object, "tracking", _soundclass];
            continue;
        };
        case ("marked" in _type): {
            [_heli, _object] call fza_ase_fnc_classification params ["_soundclass","_iconClass"];
            _rwrObjects pushBack [ASE_ACQ, getpos _object, _iconClass];
