...
if (!_hydFailure || _emerHydOn) then {
    if (fza_ah64_sfmPlusControlScheme == KEYBOARD || fza_ah64_sfmPlusControlScheme == MOUSE) then {
        if (_keyCollectiveUp > 0.1) then { _collectiveValue = _collectiveValue + (((1.0 * fza_ah64_sfmPlusKeyboardSense) / 3.0) * _deltaTime); };
        if (_keyCollectiveDn > 0.1) then { _collectiveValue = _collectiveValue - (((1.0 * fza_ah64_sfmPlusKeyboardSense) / 3.0) * _deltaTime); };
