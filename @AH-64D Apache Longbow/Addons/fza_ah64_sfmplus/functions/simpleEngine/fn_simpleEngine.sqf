...
switch (_state) do {
	case "OFF": {
        ...
        //Ng
        _ng_pct          = [_ng_pct, 0.0, _idleDelayMax * 2.5, _idleDelayMax * 5.0] call fza_sfmplus_fnc_expSeek;
        //TGT
        _coreTemp_degC   = [_coreTemp_degC, _FAT, 300, 400] call fza_sfmplus_fnc_expSeek;
        ...
        _tgt_degC        = [_tgt_degC, _tgtGoal, _tgtDelay, _tgtDelay * 1.2] call fza_sfmplus_fnc_expSeek;
        //Torque
        _outputTq        = [_outputTq, 0.0, _tqDelay, _tqDelay] call fza_sfmplus_fnc_expSeek;
    ...
    case "MOTOR": {
        ...
        //Ng
        _ng_pct          = [_ng_pct, _ng_start, _idleDelayMax * 6.0, _idleDelayMax * 2.4] call fza_sfmplus_fnc_expSeek;
        //TGT
        _coreTemp_degC   = [_coreTemp_degC, _FAT, 300, 400] call fza_sfmplus_fnc_expSeek;
        private _tgt_ng  = [_tgtNgTable, _ng_pct] call fza_fnc_linearInterp select 1;
        private _tgtGoal = if (_ng_pct > 0.20) then { _tgt_ng + 0.00; } else { _tgt_ng + ((0.20 - _ng_pct) / 0.20 * _coreTemp_degC); };
        ...
        _outputTq        = [_outputTq, _tqGoal * 481.0, _tqDelay, _tqDelay] call fza_sfmplus_fnc_expSeek;
    ...
    case "START": {
        if ((_ng_pct > 0.15) && !_noFuel) then {
            if (_ng_pct < _ng_dropout) then {
                ...
                _ng_pct              = [_ng_pct, _ng_idle * 1.1, _idleDelayMax * 4.0, _idleDelayMax * 2.4] call fza_sfmplus_fnc_expSeek;
                //TGT
                _coreTemp_degC   = [_coreTemp_degC, _FAT, 300, 400] call fza_sfmplus_fnc_expSeek;
    ...
    case "ON": {
        ...
        _ng_pct              = [_ng_pct, _ng_idle + (_ng_fly - _ng_idle) * _throttlePct, _idleDelayMax, _idleDelayMax * 2.4] call fza_sfmplus_fnc_expSeek;
        //TGT
        _coreTemp_degC       = [_coreTemp_degC, _tgt_degC, 300, 400] call fza_sfmplus_fnc_expSeek;
