...
//Throttle
if (_engPowerLeverState in [PWR_LEVER_OFF, PWR_LEVER_IDLE]) then {
    _engThrottle = 0.0;
} else { _engThrottle = 1.0; };

//Tq
if (_engPowerLeverState != PWR_LEVER_OFF) then {
...
//If the engine isn't overspeed, do normal engine things
if (!_engOverspeed) then {
    _engPctTQ = (_heli getVariable "fza_sfmplus_rtrEngTq") / 481.0;
    if (_isOEI) then {
        if (_engPowerLeverState in [PWR_LEVER_OFF, PWR_LEVER_IDLE]) then {
            _engPctTQ = 0.0;
        } else {
