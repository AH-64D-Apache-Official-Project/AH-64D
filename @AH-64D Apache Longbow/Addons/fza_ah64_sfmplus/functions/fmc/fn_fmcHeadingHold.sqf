...
private _desiredHdg    = _heli getVariable "fza_sfmplus_hdgHoldDesiredHdg";
private _hdgError      = [_curHdg - _desiredHdg] call CBA_fnc_simplifyAngle180;
private _desiredSlip   = _heli getVariable "fza_sfmplus_hdgHoldDesiredSideslip";
...
private _subMode       = _heli getVariable "fza_sfmplus_hdgHoldSubMode";
private _attSubMode    = _heli getVariable "fza_sfmplus_attHoldSubMode";
...
if (_onGnd
    || _heli getVariable "fza_sfmplus_forceTrimInterupted" 
    || fza_sfmplus_pedalLeftRight <= -_breakoutValue 
    || fza_sfmplus_pedalLeftRight >=  _breakoutValue) then {
    if (_heli getVariable "fza_sfmplus_hdgHoldActive" isNotEqualTo false) then {
        _heli setVariable ["fza_sfmplus_hdgHoldActive", false, true];
    };
} else {
    if (_heli getVariable "fza_sfmplus_hdgHoldActive" isNotEqualTo true) then {
        _heli setVariable ["fza_sfmplus_hdgHoldActive", true, true];
...
if (_heli getVariable "fza_sfmplus_hdgHoldActive") then {
    ...
    if (_curVel < HDG_HOLD_SPEED_SWITCH_ACCEL) then {
        if (_subMode isNotEqualTo "hdg") then {
            _heli setVariable ["fza_sfmplus_hdgHoldSubMode", "hdg", true];
        };
    } else {
        if (_subMode isNotEqualTo "trn") then {
            _heli setVariable ["fza_sfmplus_hdgHoldSubMode", "trn", true];
        };
    };
    //Heading Hold
