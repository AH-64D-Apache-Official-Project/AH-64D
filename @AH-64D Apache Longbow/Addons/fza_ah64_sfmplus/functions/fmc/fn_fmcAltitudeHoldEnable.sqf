...
if (_heli getVariable "fza_sfmplus_altHoldActive" == false) then {
    ...
    if (_curVelZ <= 1.016 && _curVelZ >= -1.016) then {
        ...
        //de-activate.
        [_heli, "fza_sfmplus_altHoldCollRef", fza_sfmplus_collectiveOutput] call fza_fnc_updateNetworkGlobal;
        ...
        if (_curAltAGL < 435.254 && _curVel < 20.577) then {
            [_heli, "fza_sfmplus_altHoldDesiredAlt", _curAltAGL] call fza_fnc_updateNetworkGlobal;
            [_heli, "fza_sfmplus_altHoldSubMode", "rad"] call fza_fnc_updateNetworkGlobal;
        } else {
            [_heli, "fza_sfmplus_altHoldDesiredAlt", _curAltASL] call fza_fnc_updateNetworkGlobal;
            [_heli, "fza_sfmplus_altHoldSubMode", "bar"] call fza_fnc_updateNetworkGlobal;
        };
        //Activate altitude hold
        [_heli, "fza_sfmplus_altHoldActive", true] call fza_fnc_updateNetworkGlobal;
    };
} else {
    ...
    [_heli, "fza_sfmplus_altHoldCollRef", 0.0] call fza_fnc_updateNetworkGlobal;
