...
private _CYSigma = 0.0; private _aI = 0.0; private _dragH = 0.0; private _sideJ = 0.0;
...
_aI_3 = if (_gamma == 0 || _omega == 0.0) then { 0.0; } else { 24.0 * _q_w / ((B * B * B * B) * _gamma * _omega); };
...
_CYSigma_4 = (rad _a0_deg) * (rad _a1_deg) * (_mu * _mu);
...
_sideJ   = _b * _c * _R * _rho * (_omegaR * _omegaR) * _CYSigma;
