...
//SH79, eqn 32
private _a1_1 = 1.0 / (1.0 - (_mu * _mu) / (2.0 * (B * B)));
private _a1_2 = (2.0 * _lambda + (8.0 / 3.0) * (rad _theta75_deg)) * _mu;
private _a1_3 = if (_omega == 0) then { 0.0; } else { _p_w / _omega; };
private _a1_4 = if (_gamma == 0 || _omega == 0) then { 0.0; } else { 16.0 * _q_w / ((B * B * B * B) * _gamma * _omega); };
...
private _b1_4 = if (_gamma == 0 || _omega == 0) then { 0.0; } else { 16.0 * _q_w / ((B * B * B * B) * _gamma * _omega); };
