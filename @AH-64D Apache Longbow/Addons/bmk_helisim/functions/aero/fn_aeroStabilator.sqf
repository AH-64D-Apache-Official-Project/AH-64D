...
Author:
	BradMick
---------------------------------------------------------------------------- */
params ["_heli", "_deltaTime", "_controlInputs", "_stabPosition", "_stabDimensions"];

private _config = configFile >> "CfgVehicles" >> typeof _heli >> "Fza_SfmPlus"; //<-- will need to pull from helisim config

_controlInputs
    params ["_collectiveVal", "_cyclicPitchVal", "_cyclicRollVal", "_pedalVal", "_engineThrottleVal"];
_stabDimensions
    params ["_stabWidth", "_stabLength"];

if (!local _heli) exitWith {};

private _stabPvt = _stabPosition;
...
private _intStabTable = [_stabTable, _collectiveVal] call fza_fnc_linearInterp;
...
//    |             |             |
//    D-------------I-------------C
private _width  = _stabWidth;
private _length = _stabLength;
...
//TEMP
//Airfoil data = NACA0012
private _airfoilTable0 =
[
//------AoA-[0]----------CL-[1]--------CD-[2]-------------
...
private _intAirfoilTable = [_airfoilTable0, _AoA] call fza_fnc_linearInterp;
