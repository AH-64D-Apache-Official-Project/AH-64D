...
if (_engStart == true) then {
    ...
    _engOutputNg  = [_engOutputNg, _startNg, (1 / 5) * _deltaTime] call BIS_fnc_lerp;
    _engOutputRPM = [_engOutputRPM, 4598, (1 / 20) * _deltaTime] call BIS_fnc_lerp;
};

if (_engStart != true && _engThrottlePos == "OFF") then {
    _engOutputNg  = [_engOutputNg,  0.0, _deltaTime] call BIS_fnc_lerp;
    _engOutputRPM = [_engOutputRPM, 0.0, _deltaTime] call BIS_fnc_lerp;
};
