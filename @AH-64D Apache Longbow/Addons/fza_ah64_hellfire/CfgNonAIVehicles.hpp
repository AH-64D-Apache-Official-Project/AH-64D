class CfgNonAIVehicles {
    class ProxyWeapon;
    
    class Proxyfza_agm114fa: ProxyWeapon
    {
        model = "\fza_ah64_model\fza_agm114fa.p3d";
        simulation = "maverickweapon";
    };
    class Proxyfza_agm114k: ProxyWeapon
    {
        model = "\fza_ah64_model\fza_agm114k.p3d";
        simulation = "maverickweapon";
    };
    class Proxyfza_agm114k2a: ProxyWeapon
    {
        model = "\fza_ah64_model\fza_agm114k2a.p3d";
        simulation = "maverickweapon";
    };
    class Proxyfza_agm114l: ProxyWeapon
    {
        model = "\fza_ah64_model\fza_agm114l.p3d";
        simulation = "maverickweapon";
    };
    class Proxyfza_agm114n: ProxyWeapon
    {
        model = "\fza_ah64_model\fza_agm114n.p3d";
        simulation = "maverickweapon";
    };
    class Proxyfza_rail_ll: ProxyWeapon {
        model = \fza_ah64_model\weps\pylons\fza_rail_ll.p3d;
        simulation = "pylonpod";
    };
    class Proxyfza_rail_lr: ProxyWeapon {
        model = \fza_ah64_model\weps\pylons\fza_rail_lr.p3d;
        simulation = "pylonpod";
    };
    class Proxyfza_rail_ul: ProxyWeapon {
        model = \fza_ah64_model\weps\pylons\fza_rail_ul.p3d;
        simulation = "pylonpod";
    };
    class Proxyfza_rail_ur: ProxyWeapon {
        model = \fza_ah64_model\weps\pylons\fza_rail_ur.p3d;
        simulation = "pylonpod";
    };
};