...
private _largeAmmoClass = "ammo_Missile_mim145";
private _mediumAmmoClass = "R_MRAAWS_HE_F";
private _smallAmmoClass = "M_SPG9_HEAT";
...
if (_system == "hit_msnEquip_pylon1" && _damage > SYS_WPN_DMG_THRESH) then {
    ...
    if (_heli getvariable "fza_sfmplus_stn1FuelMass" < 200) then {_ammoClass = _mediumAmmoClass;};
...
if (_system == "hit_msnEquip_pylon2" && _damage > SYS_WPN_DMG_THRESH) then {
    ...
    if (_heli getvariable "fza_sfmplus_stn2FuelMass" < 200) then {_ammoClass = _mediumAmmoClass;};
...
if (_system == "hit_msnEquip_pylon3" && _damage > SYS_WPN_DMG_THRESH) then {
    ...
    if (_heli getvariable "fza_sfmplus_stn3FuelMass" < 200) then {_ammoClass = _mediumAmmoClass;};
...
if (_system == "hit_msnEquip_pylon4" && _damage > SYS_WPN_DMG_THRESH) then {
    ...
    if (_heli getvariable "fza_sfmplus_stn4FuelMass" < 200) then {_ammoClass = _mediumAmmoClass;};
