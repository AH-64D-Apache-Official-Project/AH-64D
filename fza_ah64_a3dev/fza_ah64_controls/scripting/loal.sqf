...
if (count (nearestObjects [[_x,_y,0], ["Car","Tank","Ship","LaserTarget"], 50]) > 0 || _curalt > _alt + 500) exitWith {_minangle = _angle};
_pbvar = _missobj call fza_ah64_getpb;
_dir = direction _missobj;
_angle = _pbvar select 0;
_angle = _angle + 2;
if(_angle > 30) then {_angle = 30;};
...
while {(alive _missobj)} do
{
if (_angle < (_minangle * -1)) exitWith {_missobj setvelocity [0,0,0];};
_pbvar = _missobj call fza_ah64_getpb;
_dir = direction _missobj;
_angle = _pbvar select 0;
_angle = _angle - 2;
