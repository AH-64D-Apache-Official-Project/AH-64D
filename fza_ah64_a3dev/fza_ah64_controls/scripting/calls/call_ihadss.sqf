...
_rocketcode = "???";
_targrange = "0";
if(isNil "fza_ah64_shottimer") then {fza_ah64_shottimer = 0;};
_laseron = 0;
///NAV/////
_waypointcode = "";
_gspdcode = "";
...
((uiNameSpace getVariable "fza_ah64_raddisp")displayCtrl 123) ctrlCommit 0.01;
((uiNameSpace getVariable "fza_ah64_raddisp")displayCtrl 124) ctrlsetposition [0.4, 0.7, 0.5, 0.12];
((uiNameSpace getVariable "fza_ah64_raddisp")displayCtrl 124) ctrlCommit 0.01;
((uiNameSpace getVariable "fza_ah64_raddisp")displayCtrl 125) ctrlsetposition [0.15, 0.7, 0.5, 0.12];
((uiNameSpace getVariable "fza_ah64_raddisp")displayCtrl 125) ctrlCommit 0.01;
//hide these
...
{
1 cutrsc ["fza_ah64_raddisp", "PLAIN",0.01];
...
if ((gunner _heli == player || driver _heli == player) && fza_ah64_ihadssoff == 0) then
{

if((isNull (uiNameSpace getVariable "fza_ah64_raddisp")) && (fza_ah64_apuon == 1 || fza_ah64_pwron == 1 || isengineon _heli || _heli animationphase "plt_eng1_start" > 0 || _heli animationphase "plt_eng2_start" > 0 || !(fza_ah64_cem))) then
{
1 cutrsc ["fza_ah64_raddisp", "PLAIN",0.01];
...
if (!(vehicle player isKindOf "fza_ah64base")) then {1 cuttext ["", "PLAIN"]; 2 cuttext ["", "PLAIN"]; };
};
...
if (fza_ah64_agmode == 3) then {_sensor = "R "; _sensxm = "FCR/G "; fza_ah64_agmode = 0;};
...
if (difficultyEnabledRTD && count (enginesTorqueRTD _heli) > 0) then {_collective =  format ["%1",round (((enginesTorqueRTD _heli select 0) + (enginesTorqueRTD _heli select 1)) / 35)];};
...
{
if(fza_ah64_shottimer == 0) then {fza_ah64_shottimer = fza_ah64_firedist;};
fza_ah64_shottimer = fza_ah64_shottimer - 0.3;
if(fza_ah64_shottimer < 0) then {fza_ah64_shottimer = 0;};
...
if (_x isKindOf "helicopter") then {_type = "\fza_ah64_US\tex\ICONS\ah64_hc_pfz.paa";};
if (_x isKindOf "plane") then {_type = "\fza_ah64_US\tex\ICONS\ah64_ac_pfz.paa";};
if (_x isKindOf "tank") then {_type = "\fza_ah64_US\tex\ICONS\ah64_tnk_pfz.paa";};
if (_x isKindOf "car") then {_type = "\fza_ah64_US\tex\ICONS\ah64_whl_pfz.paa";};
if (_adaunit) then {_type = "\fza_ah64_US\tex\ICONS\ah64_ada_pfz.paa";};
if (!(_x isKindOf "car" || _x isKindOf "tank" || _x isKindOf "plane" || _x isKindOf "helicopter" || _adaunit)) then {_type = "\fza_ah64_US\tex\ICONS\ah64_gen_pfz.paa";};
...
//auto hover detector//
if((inputaction "HeliBack" > 0.25 || inputaction "HeliForward" > 0.25 || inputaction "HeliFastForward" > 0.25 || inputaction "HeliCyclicLeft" > 0.25 || inputaction "HeliCyclicRight" > 0.25) && (isAutoHoverOn _heli && player == driver _heli)) then
{
...
if(cameraView == "GUNNER" && player == gunner _heli) then
{
	...
	{
		((uiNameSpace getVariable "fza_ah64_raddisp")displayCtrl 123) ctrlSetText "DTV";
	} else {
		((uiNameSpace getVariable "fza_ah64_raddisp")displayCtrl 123) ctrlSetText "FLIR";
	};
} else {
...
if (vehicle player != _heli && !(vehicle player isKindOf "fza_ah64base") || !(alive _heli) && !(vehicle player isKindOf "fza_ah64base") || !(alive player)) then {1 cuttext ["", "PLAIN"]; 2 cuttext ["", "PLAIN"]; fza_ah64_bweff ppEffectEnable false; fza_ah64_fgeff ppEffectEnable false;};